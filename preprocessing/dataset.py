...
class TraceDataset(Dataset):
    def __init__(self, df):
        ...
        self.labels = df[["labels_orientation", "labels_channel", "isGenuine"]]
    ...
    def fromOneHot(self, y):
        ...
        orientation_index, channel_index, isGenuine = np.split(y,[len(self.orientation), len(self.channel) + len(self.orientation)], axis=1)
        
        y_orientation = _fromOneHot(orientation_index, self.orientation)
        y_channel = _fromOneHot(channel_index, self.channel)
        return y_orientation, y_channel, np.squeeze(isGenuine.tolist())
    ...
    def __getitem__(self, idx):
        ...
        y = [*y_orientation, *y_channel, y["isGenuine"].astype(int)]
...
dataloader = DataLoader(trace_dataset, batch_size=32, shuffle=False)
i = iter(dataloader)
data = next(i)
